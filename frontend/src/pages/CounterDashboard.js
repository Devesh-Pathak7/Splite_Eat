                ...
                return (
                  <Card key={group.session_id || mainOrder.id} className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md hover:shadow-lg transition-all" data-testid={`order-${mainOrder.id}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {mainOrder.status === 'PREPARING' && (
                        <Button size="sm" onClick={() => updateOrderStatus(mainOrder.id, 'READY')} className="bg-green-500 hover:bg-green-600" data-testid={`order-${mainOrder.id}-ready-btn`}>
                          Mark Ready
                        </Button>
                      )}
                      {mainOrder.status === 'READY' && (
                        <Button size="sm" onClick={() => updateOrderStatus(mainOrder.id, 'SERVED')} className="bg-purple-500 hover:bg-purple-600" data-testid={`order-${mainOrder.id}-served-btn`}>
                          Mark Served
                        </Button>
                      )}
                      {mainOrder.status === 'SERVED' && (
